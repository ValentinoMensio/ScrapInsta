...
def wait_workers_ready(workers_ready, expected, timeout=600, stop_event=None):
    """Espera a que todos los workers anuncien ready (una vez cada uno). Cancelable con stop_event."""
    ready = 0
    deadline = time.time() + timeout
    while ready < expected and time.time() < deadline:
        if stop_event is not None and stop_event.is_set():
            logger.info("Stop solicitado durante wait_workers_ready; abortando espera…")
            break
        try:
...
def shutdown(processes, worker_queues, result_queue=None, workers_ready=None, timeout=10, stop_event=None):
    ...
    # Dar tiempo a que terminen solos
    for p in processes:
        try:
            p.join(timeout)
        except Exception:
    ...
    # Cerrar también colas compartidas si existen
    for q in (result_queue, workers_ready):
        if q is None:
            continue
        try:
            q.close()
...
def main():
    ...
    # Solo el padre maneja SIGINT/SIGTERM. (En los workers, IGNORAR SIGINT dentro de worker_entry)
    ...
    try:
        ...
        if not wait_workers_ready(workers_ready, expected=len(accounts), timeout=600, stop_event=stop_event):
            logger.error("No todos los workers anunciaron readiness a tiempo")
            stop_event.set()
            shutdown(processes, worker_queues, result_queue, workers_ready, stop_event=stop_event)
            return
        ...
        while True:
            # Corte inmediato si se pidió stop (por Ctrl+C o SIGTERM)
            if stop_event.is_set():
                logger.info("Stop solicitado; apagando…")
                break
        ...
        # Normalmente no llega acá porque manejamos SIGINT con signal.signal,
